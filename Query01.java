...
class CreateFirstForm {
	...
	CreateFirstForm() {
		...
		jsp1 = new JScrollPane(jlist1, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
		...
		jsp2 = new JScrollPane(jlist2, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
				JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
...
cff.jlist2.setListData(Connect.fieldNames); //will set column names into jlist


try
{

Connection c=DriverManager.getConnection(Connect.first,"root","mca6");
...
cff.jlist1.setListData(Connect.columnNames); //will set column names into list
 ...
 catch(SQLException e) { System.out.println(" Query Event Table does not Exist "+e);}
 catch(Exception e) {System.out.println(" Query Event Simple Exception"+e); }
...
cff.jlist2.setListData(Connect.fieldNames); //will set column names into jlist2 cff.jlist1.clearSelection();

//System.out.println(" ~ "+sel[0]+" "+sel[1]+" "+sel[2]+" "+sel[3]+" "+sel[4]+" "+sel[5]+" "+sel[6]+"
// "+sel[7]+" "+sel[8]+" "+sel[9]+" "+" ~ ");
...
//System.out.println("-"+sel[0]+" "+sel[1]+" "+sel[2]+" "+sel[3]+" "+sel[4]+" "+sel[5]+" "+sel[6]+" "+sel171+" "+self81+" "+sel[9]+" "+" ");
}
...
{
// System.out.print(" &&& ");
if((cff.jcom2.getSelectedItem().equals("--None---")==false) && (cff.jcom3.getSelectedItem().equals("---None---")==false) && (cff.jtf1.getText().equals("Enter Values")==false))
{
...
class TableResult {
	...
	TableResult(StringBuffer sb) {
		try {
			c = DriverManager.getConnection(Connect.first,"root","mca6");
			...
			System.out.println(" TableResult Table does not Exist"+e);
		} catch (Exception e) {
			System.out.println(" TableResult Simple Exception"+e);
		}
	...
	void showTable() {
		try {
			...
			while (rs.next()) {
				...
				// System.out.print(tdb.rowDataTemp+" @@"+tdb.rowData Temp.size()+"
				// "+tdb.rowDataTemp.capacity
				// System.out.println("-"+tdb.rowDam2884"+tdb.rowDataMain.size()+"
				// "+tdb.rowDataMain.capacity());
...
class Connect {
	...
	public void connectDSN() {
		try {
			...
			//Connection c = DriverManager.getConnection(dsnName);
			first="jdbc:mysql://localhost:3306/"+dsnName;
			Connection c= DriverManager.getConnection(first,"root","mca6");
			Statement s = c.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
			...
			System.out.println(" Table does not Exist" +e);
		} catch (Exception e) {
...
class ConnectEvent implements ActionListener {
	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getSource() == con.jb01) {
			//Connect.dsnName=con.cmdDataBase.getSelectedItem().toString();
